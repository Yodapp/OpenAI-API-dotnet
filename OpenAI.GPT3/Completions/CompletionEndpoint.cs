...
namespace OpenAI.GPT3
{
    ...
    /// </summary>
    public class CompletionEndpoint : ApiEndpointBase
	{
		/// <summary>
		...
		internal CompletionEndpoint(Api api) : base(api)
		{
		}
		...
		/// <returns>Asynchronously returns the completion result.  Look in its <see cref="CompletionResponse.Choices"/> property for the completions.</returns>
		public async Task<CompletionResponse> CreateCompletionAsync(CompletionRequest request)
		{
			...
			if (response.IsSuccessStatusCode)
			{
				...
				var res = JsonSerializer.Deserialize<CompletionResponse>(resultAsString);
				try
				{
		...
		/// <returns>Asynchronously returns the completion result.  Look in its <see cref="CompletionResponse.Choices"/> property for the completions, which should have a length equal to <paramref name="numOutputs"/>.</returns>
		public Task<CompletionResponse> CreateCompletionsAsync(CompletionRequest request, int numOutputs = 5)
		{
		...
		/// <returns>Asynchronously returns the completion result.  Look in its <see cref="CompletionResponse.Choices"/> property for the completions.</returns>
		public Task<CompletionResponse> CreateCompletionAsync(string prompt,
			int? max_tokens = null,
		...
		/// <returns></returns>
		public Task<CompletionResponse> CreateCompletionAsync(params string[] prompts)
		{
			CompletionRequest request = new CompletionRequest(DefaultCompletionRequestArgs)
			{
		...
		public async Task StreamCompletionAsync(CompletionRequest request, Action<int, CompletionResponse> resultHandler)
		{
			if (_api.Auth?.ApiKey is null)
			{
				throw new AuthenticationException("You must provide API authentication.  Please refer to https://github.com/OkGoDoIt/OpenAI-API-dotnet#authentication for details.");
			...
			using (HttpRequestMessage req = new HttpRequestMessage(HttpMethod.Post, $"https://api.openai.com/v1/engines/{_api.UsingEngine.EngineName}/completions"))
			{
				req.Content = stringContent;
				req.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", _api.Auth.ApiKey); ;
				...
				if (response.IsSuccessStatusCode)
				{
					...
					using (StreamReader reader = new StreamReader(stream))
					{
						...
						while ((line = await reader.ReadLineAsync()) != null)
						{
							...
							{
								index++;
								var res = JsonSerializer.Deserialize<CompletionResponse>(line.Trim());
								try
								{
									res.Organization = response.Headers.GetValues("Openai-Organization").FirstOrDefault();
									res.RequestId = response.Headers.GetValues("X-Request-ID").FirstOrDefault();
		...
		public async Task StreamCompletionAsync(CompletionRequest request, Action<CompletionResponse> resultHandler)
		{
		...
		/// <summary>
		/// Ask the API to complete the prompt(s) using the specified request, and stream the results to the <paramref name="resultHandler"/> as they come in.
		/// If you are not using C# 8 supporting async enumerables or if you are using the .NET Framework, you may need to use <see cref="StreamCompletionAsync(CompletionRequest, Action{CompletionResponse})"/> instead.
		/// </summary>
		/// <param name="request">The request to send to the API.  This does not fall back to default values specified in <see cref="DefaultCompletionRequestArgs"/>.</param>
		...
		public async IAsyncEnumerable<CompletionResponse> StreamCompletionEnumerableAsync(CompletionRequest request)
		{
			if (_api.Auth?.ApiKey is null)
			{
				throw new AuthenticationException("You must provide API authentication.  Please refer to https://github.com/OkGoDoIt/OpenAI-API-dotnet#authentication for details.");
		...
		public IAsyncEnumerable<CompletionResponse> StreamCompletionEnumerableAsync(string prompt,
			int? max_tokens = null,
			double? temperature = null,
			double? top_p = null,
			int? numOutputs = null,
